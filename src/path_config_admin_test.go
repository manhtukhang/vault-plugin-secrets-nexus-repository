...
func TestConfigAdmin_SimpleCRUD(t *testing.T) {
	...
	// Read admin config again to verify the written values
	resp, err = doAction(actionRead, configAdminPath, b, reqStorage, nil)
	...
	assert.Equal(t, testConfigAdminInsecureUpdate, resp.Data["insecure"]) // default value
	assert.Equal(t, testConfigAdminTimeoutUpdate, resp.Data["timeout"])   // default value
...
func TestConfigAdmin_Create_MissingRequireFields(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		assert.Equal(t, resp.Error().Error(), fmt.Sprintf(`missing "%s" in admin configuration`, tc.missingField))
	}
}
